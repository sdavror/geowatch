...
## Built this round (`feature/registry-expansion`) — all three ingested live against production data
...
## Researched, needs a self-service account (not built blind)

| Source | Country/scope | Type | Notes |
|---|---|---|---|
| US Consolidated Screening List (trade.gov) | US, aggregates 11 lists (OFAC SDN/Non-SDN, BIS Entity List, Denied Persons List, Unverified List, DDTC debarred, etc.) | sanctions | Highest-leverage candidate on this list — one feed instead of many. Requires signing up for a free key at developer.trade.gov (account creation), which needs the user's own action, not something to do on their behalf. Endpoint/auth researched (`api.trade.gov/consolidated_screening_list/v1/search`, `subscription-key` header) but the exact response field names weren't confirmed live — **not built** rather than shipping unverified guesses at the JSON shape. Build once a key exists. |
